...
const MobileNav = ({showNav, closeNav} : Props) => {
  ...
  return (
    <div>
      ...
      {/* navlinks */}
      <div className={`text-white ${navOpen} fixed justify-center flex flex-col h-full   transform transition-all duration-500 delay-300 w-[80%] sm:[60%] bg-orange-500 space-y-6 z-[1050]`}>
        {navLinks.map((link) => {
            return (
        ...
        {/* close icon */}
      <CgClose onClick={closeNav} className='absolute top-[0.7rem] right-[1.4rem] sm:size-8 size-6'/>
      </div>
